...
class Soft_Aux:
    ...
    def __init__(self):
        ...
        print('Building Soft_Auxiliary BiLSTM Models ...')
    ...
    def fit(self, X_train, y_train, X_valid, y_valid, embedding_layer):
        ...
        self.model.compile(loss = "categorical_crossentropy",\
                           optimizer = Adam(lr = self.arguments['learning_rate'],\
                                            decay = self.arguments['learning_rate_decay']),\
                           metrics = ["accuracy"])
        history = self.model.fit(X_train, y_train, batch_size = self.arguments['batch_size'],\
                                 epochs = self.arguments['epochs'],\
                                 validation_data = (X_valid, y_valid), verbose = 1,\
                                 callbacks = [ra_val, check_point, early_stop])
        self.model = load_model(file_path)

        print('Finished Building Soft_Aux BiLstm Model as class attribute class.model')
        return self
